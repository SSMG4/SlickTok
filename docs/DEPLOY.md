...
under PM2, with GitHub-webhook auto-deploy on every push to `main`.
...
SlickTok shells out to `yt-dlp` to resolve TikTok links. Debian's
...
- `PORT` — leave as `3000` unless it conflicts with something else
...
- `DEPLOY_WEBHOOK_SECRET` — generate one and keep it handy, you'll
  ...
  openssl rand -hex 32
  ```
- `DEPLOY_BRANCH` — leave as `main` unless you deploy from elsewhere.
...
You should see `SlickTok listening on http://localhost:3000`. In a
...
- **`{"ok":true}` fails locally but works over PuTTY's own curl**:
  make sure you're testing `localhost:3000` on the server itself, not
